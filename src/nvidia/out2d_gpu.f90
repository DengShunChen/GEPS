      subroutine out2d_gpu (nx,lev,my,my_max,itau,idtg,taudir,ntau         &
...
!$acc wait(async_id)
!$acc enter data create(globp,mout) async(async_id)
...
!surface albedo  0 - 1.0
      ...
      do jj=1,jlistnum
       do  i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj)  globp(i,jj)=alb(i,jj)
       enddo 
      enddo 
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!cumulus parameterization precipitation
      ...
      do jj=1,jlistnum
       do  i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=raincu(i,jj)
       enddo 
      enddo 
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!large scale precipitation 
      ...
      do jj=1,jlistnum
       do  i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=rainlp(i,jj)
       enddo 
      enddo 
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!total precipitation  every 12 hour reset to zero (mm)
      ...
      do 98 i=1,nxp
         j=jlist1(jj)
         nxj=nxdef_2d(j)
         if(i<=nxj) globp(i,jj)=raincu(i,jj)+rainlp(i,jj)
 98   continue
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
      call qmaxn3_w (glob,1,1,1,nx,my,1)
...
!
      ...
      do jj=1,jlistnum
       do  i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=raintot(i,jj)
       enddo 
      enddo 
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!runoff
      ...
      do jj=1,jlistnum
       do  i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=runoff(i,jj)
       enddo 
      enddo 
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!atmosphere column precipitable water (mm)
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,4),glob)
...
!net radiation flux at the surface
      ...
      do i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=ss(i,jj)-rs(i,jj)
      enddo
      enddo
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!net longwave flux at the top atmosphere
      ...
      do jj=1,jlistnum
       do i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=cumtop(i,jj)
       enddo
      enddo
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!sensible heat flux at the surface
      ...
      do jj=1,jlistnum
       do i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=hflux(i,jj)
       enddo
      enddo
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!lentent heat flux at the surface
      ...
      do jj=1,jlistnum
       do i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=qflux(i,jj)
       enddo
      enddo
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!land suface tempaerature or sea surface temperature
      ...
      do jj=1,jlistnum
       do i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=tg(i,jj)
       enddo
      enddo
      call unify_reduceintp_idw_gpu(nx,my,my_max,globp,glob)
...
!land skin air temperature (the model lowest)
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,1),glob)
...
!skin u component ( model lowest)
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,2),glob)
...
!skinvu component ( model lowest)
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,3),glob)
...
!skin relative humidity (model lowest)
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,5),glob)
...
!2m relative humidity
      ...
      do 37 i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=rh2(i,jj) * 100.0
 37   continue
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!10m relative humidity
      ...
      do 38 i=1,nxp
         j=jlist1(jj)
       nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=rh10(i,jj) * 100.0
 38   continue
      call unify_reduceintp_gpu(nx,my,my_max,globp,glob)
...
!ground wetness 0 - 1.0
      ...
      do 36 i=1,nxp
         j=jlist1(jj)
       nxj=nxdef_2d(j)
       if(i<=nxj) globp(i,jj)=gwet(i,jj)/20.
 36   continue
      call unify_reduceintp_idw_gpu(nx,my,my_max,globp,glob)
...
!canopy moisture content(mm)
      ...
      do jj=1,jlistnum
       do i=1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
        if(i<=nxj) globp(i,jj)=canopy(i,jj)
       enddo
      enddo
      call unify_reduceintp_idw_gpu(nx,my,my_max,globp,glob)
...
! 0-10cm Volumetric soil moisture fraction (0-1.0)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,1),glob)
...
! 10-40cm Volumetric soil moisture fraction (0-1.0)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,2),glob)
...
! 40-100cm Volumetric soil moisture fraction (0-1.0)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,3),glob)
...
! 100-200cm Volumetric soil moisture fraction (0-1.0)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,4),glob)
...
! 0-10cm Unfrozen(liquid) soil moisture content(volumetric fraction)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,5),glob)
...
! 10-40cm Unfrozen(liquid) soil moisture content(volumetric fraction)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,6),glob)
...
! 40-100cm Unfrozen(liquid) soil moisture content(volumetric fraction)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,7),glob)
...
! 100-200cm Unfrozen(liquid) soil moisture content(volumetric fraction)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,8),glob)
...
! 0-10cm Volumetric soil temperature(K)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,9),glob)
...
! 10-40cm Volumetric soil temperature(K)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,10),glob)
...
! 40-100cm Volumetric soil temperature(K)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,11),glob)
...
! 100-200cm Volumetric soil temperature(K)
      ...
      call unify_reduceintp_idw_gpu(nx,my,my_max,soil_xy(1,1,12),glob)
...
! ctot_total cloud fraction
      ...
      if( itau==0 .or. itau .gt. nint(domfc) )then
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,6),glob)
...
! chig_high cloud fraction
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,7),glob)
...
! cmid_middle cloud fraction
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,8),glob)
...
! clow_low cloud fraction
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,9),glob)
...
! pbl height
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,10),glob)
...
! specific humidity 
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,11),glob)
...
!
      ...
      call unify_reduceintp_gpu(nx,my,my_max,wk_xy(1,1,12),glob)
...
!      call unify_reduceintp_gpu(nx,my,my_max,flash,glob)
...
!$acc wait(async_id)
!$acc exit data delete(globp,mout) async(async_id)
!$acc wait(async_id)
!
      return
      end
