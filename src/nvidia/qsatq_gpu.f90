...
contains
      ...
      subroutine qsatq_gpu (nx,my_max,tqs,pqs,qss)
!
...
!
      ...
      integer   im,jm,i,j,jj,ic
      integer   nx,nxj,my_max

      real      tqs(nxp,my_max),pqs(nxp,my_max),qss(nxp,my_max)
      real      tem,epsm1,t1,qqq,fpvs_1
...
!
!
!$acc enter data copyin ( vpsat ) async(async_id)
!$acc wait(async_id)

      tem   = 1.0/1.622
      epsm1 = 0.622-1.0
!$acc parallel loop collapse(2) private(j,nxj,xj,jx,fpvs_1,qqq ) async(async_id)
      do jj = 1, jlistnum
       do i = 1,nxp
        j=jlist1(jj)
        nxj=nxdef_2d(j)
...
! fpvs qpv_sat(in unit pa)
        xj=min(max( c1xpvs + c2xpvs * tqs(i,jj) , 1.0 ),real(nxpvs))
        jx=min(xj,nxpvs-1.0)
        fpvs_1=tbpvs(jx)+(xj-jx)*(tbpvs(jx+1)-tbpvs(jx))

        qqq = min ( pqs(i,jj) , 0.01 * fpvs_1 )
!
!       qss(i) = 0.622*qqq/(pqs(i)-qqq)
        qss(i,jj) = 0.622*qqq/(pqs(i,jj)+epsm1*qqq)
       endif
      enddo
     enddo
  100 continue
!$acc wait(async_id)
!$acc exit data delete ( vpsat ) async(async_id)
!$acc wait(async_id)
      return

      end

      subroutine qsatq_3d_gpu (nx,my_max,lev,tqs,pqs,qss)
!
!  vectorized saturation specific humidity subroutine
!  algorithm is table-lookup.
!
...
!
      ...
      integer   im,lev,k,i,ic
      integer   nx,j,jj,nxj,my_max

      real      tqs(nxp,lev,my_max),pqs(nxp,lev,my_max),qss(nxp,lev,my_max)
!      real      vpsat(191)
...
!$acc wait(async_id)
      tem = 1.0/1.622
      epsm1=0.622-1.
!$acc parallel loop collapse(3) private(j,nxj,xj,jx,fpvs_1,qqq ) async(async_id)
      do jj = 1, jlistnum
       do k=1,lev
       do i = 1,nxp
        j=jlist1(jj)
...
!                                     *(t1-float(ic)))
! fpvs qpv_sat(in unit pa)
        xj=min(max( c1xpvs + c2xpvs * tqs(i,k,jj) , 1.0 ),real(nxpvs))
        jx=min(xj,nxpvs-1.0)
        fpvs_1=tbpvs(jx)+(xj-jx)*(tbpvs(jx+1)-tbpvs(jx))

        qqq = min ( pqs(i,k,jj) , 0.01*fpvs_1 )
!
!        qss(i,k) = 0.622*qqq/(pqs(i,k)-qqq)
        qss(i,k,jj) = 0.622*qqq/(pqs(i,k,jj)+epsm1*qqq)
       endif
      enddo
      enddo
     enddo
