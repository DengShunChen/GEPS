...
!CWB2015 <<<
   ...
   !$acc data copy(ws3, ws4, div, vor, tcc, tc2, wcc_fk, jlist_fj, fj_ws3, fj_ws4, fj_tcc, fj_wc, fj_wd, fj_tc2) async(1)
   do m = 1, mlistnum
      mf = mlist(m)
      do j = 1, myhalf
      ...
      !$acc data copy(jlistnum_fj, mtrundef) async(1)
      !$acc kernels async(1)
      !$acc loop collapse(2)
      do l = mf, jtrun
      do k = 1, lev
...
!
   ...
   !$acc data copy(cc, jlist1, mtrundef, nlist)
   do jj = 1, jlistnum
      !$acc kernels present(cc)
      do i = 1, (nx + 2)*lev*2
         cc(i, 1, 1, jj) = 0.
      end do
      !$acc end kernels
