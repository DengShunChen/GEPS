...
!=lzl+add---------------------------------------------------------
      ...
      real wk(ix,my_max)
      real bnv2lm(ix,km,my_max),pe(ix,my_max),ek(ix,my_max),zbk(ix,my_max),up(ix,my_max)
      real db(ix,km,my_max),ang(ix,km,my_max),uds(ix,km,my_max)
...
!lzl+add-----------------------------------------------------------------
!
      real taub(ix,my_max),  xn(ix,my_max),     yn(ix,my_max),    ubar(ix,my_max)                     &
           , vbar(ix,my_max),  ulow(ix,my_max),   oa(ix,my_max),    clx(ix,my_max)                    &
           ,           roll(ix,my_max),  uloi(ix,my_max),   dusfc(ix,my_max), dvsfc(ix,my_max)        &
           ,           dtfac(ix,my_max), xlinv(ix,my_max),  delks(ix,my_max), delks1(ix,my_max)     
!
      real bnv2(ix,km,my_max),  taup(ix,km+1,my_max), ri_n(ix,km,my_max)                       &
           ,             taud(ix,km,my_max),  ro(ix,km,my_max)          &
           ,             scor(ix,my_max),      velco(ix,km-1,my_max)      &
           ,             bnv2bar(ix,my_max)
!lzl-end-----------------------------------------------------------------
!
...
! for lm mtn blocking
      integer   kreflm(ix,my_max), iwklm(ix,my_max)
...
!       cdmb = 192.0/float(imx)
        !$acc enter data create(db, ang, uds, npt, ipt,&
        !$acc&                  iwklm, kreflm, ro, bnv2lm, delks, &
        !$acc&                  delks1, ubar, vbar, roll, pe, ek, bnv2bar, &
        !$acc&                  up, taup, ri_n, bnv2, iwk, kbps, kmps, kref,&
        !$acc&                  nwdir, oa, clx, xn, yn, taub, ulow, dtfac, &
        !$acc&                  uloi, velco, kint, xlinv, scor, taud, &
        !$acc&                  utendform, vtendform) async(async_id)
...
!
      !$acc host_data use_device(db, ang, uds, taup)
...
!     ncnt   = 0
      ...
      !$acc parallel loop private(j1,i) async(async_id)
      do jj = 1, jlistnum
        j1 = jlist1(jj)
!
!xb118---for TOFD
...
!   initialize dividing streamline (ds) control vector
  !
      if ( nmtvr .eq. 14) then
        !$acc parallel loop gang private(i) async(async_id)
        do jj = 1, jlistnum
          !$acc loop vector
          do i=1,npt(jj)
            iwklm(i,jj)  = 2
...
!
      if ( nmtvr .eq. 14) then 
        !$acc parallel loop collapse(2) gang private(i,j,k,pkp1log,pklog,vtjp) async(async_id)
        do jj = 1, jlistnum
          do k = 1,kmll
            !$acc loop vector
            do i = 1, npt(jj)
...
!      &          kdt,ihit,jhit,phil(jhit,ihit),me
      klevm1 = kmll - 1
      if ( nmtvr .eq. 14) then 
        !$acc parallel loop collapse(2) gang private(i,j,k,rdz) async(async_id)
        do jj = 1, jlistnum
          do k = 1, klevm1  
            !$acc loop vector
            do i = 1, npt(jj)
...
! ---   iwklm(i) is the k-index of mtn elvmax elevation
      if ( nmtvr .eq. 14) then 
        !$acc parallel loop collapse(2) gang private(ktrial,i) async(async_id)
        do jj = 1, jlistnum         
          do ktrial = kmll, 1, -1
            !$acc loop vector
            do i = 1, npt(jj)
               if ( ktrial .lt. iwklm(i,jj) .and. kreflm(i,jj) .eq. 0 ) then
                  kreflm(i,jj) = ktrial
               endif
            enddo
          enddo
...
!   
      if ( nmtvr .eq. 14) then
        ...
        !$acc&                           rolltmp,bnv2bartmp) async(async_id)
        do jj = 1, jlistnum
          do i = 1, ix
            if (i .le. npt(jj)) then
              ubartmp = 0.
  ...
  ! ---   these vert ave are for diags, testing and gwd to follow (*j*).
              enddo
              ubar(i,jj)    = ubar(i,jj)  + ubartmp ! trial mean u below 
              vbar(i,jj)    = vbar(i,jj)  + vbartmp ! trial mean v below 
              roll(i,jj)    = roll(i,jj)  + rolltmp ! trial mean ro below 
...
!
      if ( nmtvr .eq. 14) then 
        !$acc parallel loop collapse(2) private(i,j,k,phiang) async(async_id)
        do jj = 1, jlistnum
          do i = 1, ix
            if (i .le. npt(jj)) then
              j = ipt(i,jj)
              ...
              do k = iwklm(i,jj), 1, -1
                ...
                ang(i,k,jj) = ( theta(j,jj) - phiang )
                if ( ang(i,k,jj) .gt.  90. ) ang(i,k,jj) = ang(i,k,jj) - 180.
                if ( ang(i,k,jj) .lt. -90. ) ang(i,k,jj) = ang(i,k,jj) + 180.
                ang(i,k,jj) = ang(i,k,jj) * deg_to_rad
  !
                uds(i,k,jj) =                                                  &
                   max(sqrt(u1(j,k,jj)*u1(j,k,jj) + v1(j,k,jj)*v1(j,k,jj)), minwnd)
  ! ---   test to see if we found zb previously
                if (idxzb(i,jj) .eq. 0 ) then                                  
                  pe(i,jj) = pe(i,jj) + bnv2lm(i,k,jj) *                             &
  ...
  ! ---   theta ranges from -+90deg |_ to the mtn "largest topo variations"
                  up(i,jj)  =  uds(i,k,jj) * cos(ang(i,k,jj))
                  ek(i,jj)  = 0.5 *  up(i,jj) * up(i,jj) 

  ! ---   dividing stream line  is found when pe =exceeds ek.
                  if ( pe(i,jj) .ge.  ek(i,jj) ) then
                    idxzb(i,jj) = k
                  endif
...
! 
      if ( nmtvr .eq. 14) then 
        !$acc parallel loop gang private(i,j,k,zlen,r,dbtmp) async(async_id)
        do jj = 1, jlistnum
          !$acc loop vector
          do i = 1, npt(jj)
            j = ipt(i,jj)
...
! ---   lm eq 14:
                  r = (cos(ang(i,k,jj))**2 + gamma(j,jj) * sin(ang(i,k,jj))**2) /   &
                     (gamma(j,jj) * cos(ang(i,k,jj))**2 + sin(ang(i,k,jj))**2)
! ---   (negitive of db -- see sign at tendency)
                  dbtmp = 0.25 *  cdmb *                                  &
                         max( 2. - 1. / r, 0. ) * sigma(j,jj) *              &
                         max(cos(ang(i,k,jj)), gamma(j,jj)*sin(ang(i,k,jj))) *     &
                         zlen / hprime(j,jj) 
                  db(i,k,jj) =  dbtmp * uds(i,k,jj) * ro(i,k,jj) 
!
        ...
        do k = 1,kmm1
          do i =1, ix
            if (i .le. npt(jj)) then
              ...
              vtk1  = vtj1 / prslk(j,k+1,jj)
              if (k .eq. 1) then
                ro(i,k,jj)   = rdi * prsl(j,k,jj) / vtj0 ! density tons/m**3
              endif
...
!        print *,' in gwdps_lm.f gwd:15  =',kbps,kmps
      ...
      !$acc parallel loop private(j,wdir,idir,nwd) async(async_id)
      do jj = 1, jlistnum
...
!
      !$acc parallel loop private(i) async(async_id)
      do jj = 1, jlistnum
        do i = 1,npt(jj)
          xn(i,jj)     = 0.0
          yn(i,jj)     = 0.0
...
!
          ulow(i,jj) = max(sqrt(ubar(i,jj)*ubar(i,jj) + vbar(i,jj)*vbar(i,jj)), 1.0)
          uloi(i,jj) = 1.0 / ulow(i,jj)
        enddo
      enddo
        
!
      !$acc parallel loop collapse(2) private(i,j,k) async(async_id)
      do jj = 1, jlistnum
        do  k = 1,kmm1
          do  i = 1,npt(jj)
            j            = ipt(i,jj)
            velco(i,k,jj)   = 0.5 * ((u1(j,k,jj)+u1(j,k+1,jj))*ubar(i,jj)              &
                              +  (v1(j,k,jj)+v1(j,k+1,jj))*vbar(i,jj))
            velco(i,k,jj)   = velco(i,k,jj) * uloi(i,jj)
...
!
      !$acc parallel loop private(i,j,bnv,fr,efact,coefm,tem,gfobnv,k) async(async_id)
      do jj = 1, jlistnum
        do i = 1,npt(jj)
          j      = ipt(i,jj)
          bnv    = sqrt( bnv2bar(i,jj) )
          fr     = bnv     * uloi(i,jj) * min(hprime(j,jj),hpmax)
          fr     = min(fr, frmax)
          xn(i,jj)  = ubar(i,jj) * uloi(i,jj)
          yn(i,jj)  = vbar(i,jj) * uloi(i,jj)
!
...
!
          xlinv(i,jj) = coefm * cleff
!
          tem      = fr    * fr * oc(j,jj)
          gfobnv   = gmax  * tem / ((tem + cg)*bnv)  ! g/n0
!
          taub(i,jj)  = xlinv(i,jj) * roll(i,jj) * ulow(i,jj) * ulow(i,jj)                &
                  * ulow(i,jj)  * gfobnv  * efact         ! base flux tau0
!
...
!
          ...
          scor(i,jj)  = bnv2(i,k,jj) / tem  ! scorer parameter below ref level
        enddo
      enddo
...
!
      ...
      !$acc parallel loop gang collapse(2) private(k,i) async(async_id)
      do jj = 1, jlistnum
        do k = 1, km
          if (k .le. kbps(jj)) then
            !$acc loop vector
            do i = 1,npt(jj)
              if (k .le. kref(i,jj)) taup(i,k,jj) = taub(i,jj)
            enddo
          endif
        enddo
      enddo
...
!
      ...
      !$acc parallel loop collapse(2) private(i,k,kp1,temv,scork,rscor,brvf,tem1, &
      !$acc&                      hd,fro,tem2,tem,rim,temc,taup1) async(async_id)
      do jj = 1, jlistnum
        do i = 1,ix
          if (i .le. npt(jj)) then
            !$acc loop seq
  ...
  !               tem1 = xlinv(i)*(ro(i,kp1)+ro(i,k))*brvf*velco(i,k)*0.5
                tem1 = xlinv(i,jj)*(ro(i,kp1,jj)+ro(i,k,jj))*brvf*0.5               &
...
!
      if(lcap .le. km) then
        !$acc parallel loop collapse(2) private(i,klcap,sira) async(async_id)
        do jj = 1, jlistnum
...
!
      ...
      !$acc parallel loop collapse(2) gang private(i,k) async(async_id)
      do jj = 1, jlistnum
        do k = 1,km
          !$acc loop vector
          do i = 1,npt(jj)
...
!
      ...
      !$acc parallel loop collapse(2) private(i,klcap,dtfac_min) async(async_id)
      do jj = 1, jlistnum
        do i = 1,ix
          if (i .le. npt(jj)) then
...
!       endif
      ...
      !$acc parallel loop collapse(3) gang private(i,j,k,dtaux,dtauy,&
      !$acc&                          eng0,dbim,eng1,eng2,taudtmp) async(async_id)
      do jj = 1, jlistnum
        do k = 1,km            !org 
          do i = 1,ix            !org
            if (i .le. npt(jj)) then
  ...
  !
                ...
                eng1       = 0.5*((u1(j,k,jj)+dtaux*deltim)**2.0+               &
                                 (v1(j,k,jj)+dtauy*deltim)**2.0)
              endif
              
              eng2       = 0.5*((u1(j,k,jj)+utendform(j,k,jj)*deltim)**2.0+     &
                                (v1(j,k,jj)+vtendform(j,k,jj)*deltim)**2.0)
      ...
      do jj = 1, jlistnum
        do i = 1,ix              !org
          if (i .le. npt(jj)) then
            ...
            dutmp = 0.
            dvtmp = 0.
            !!$acc loop vector reduction(+:dutmp) reduction(+:dvtmp)
            !$acc loop seq
            do k = 1,km            !org 
              delp = del(j,k,jj)
              taudtmp  = taud(i,k,jj) * dtfac(i,jj)
...
!       endif
      ...
      do jj = 1, jlistnum
        ...
        do i = 1,npt(jj)
          j          = ipt(i,jj)
!         tem    = (-1.e3/g)
          dusfc(j,jj) = tem * dusfc(j,jj)
          dvsfc(j,jj) = tem * dvsfc(j,jj)
        enddo
      enddo
...
!      &,                          b(ipt(1),idxzb(1)),me
!
      !$acc exit data delete(db, ang, uds, npt, ipt,&
      !$acc&                 iwklm, kreflm, ro, bnv2lm, delks, &
      !$acc&                 delks1, ubar, vbar, roll, pe, ek, bnv2bar, &
      !$acc&                 up, taup, ri_n, bnv2, iwk, kbps, kmps, kref,&
      !$acc&                 nwdir, oa, clx, xn, yn, taub, ulow, dtfac, &
      !$acc&                 uloi, velco, kint, xlinv, scor, taud, &
      !$acc&                 jlist1, utendform, vtendform) async(async_id)
      !$acc wait(async_id)
