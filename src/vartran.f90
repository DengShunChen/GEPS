...
!
      if(iflag.eq.2)then
         ...
         do 110 m = 1, mlistnum
          mf=mlist(m)
         do 110 l = mf, jtrun
          vorten(k,1,l,m)=-h(l,m,k)*vorten(k,1,l,m)/omega2
          vorten(k,2,l,m)=-h(l,m,k)*vorten(k,2,l,m)/omega2
          temp1=divten(k,2,l,m)/omega2
          temp2=divten(k,1,l,m)/omega2
          divten(k,1,l,m)=-h(l,m,k)*temp1
          divten(k,2,l,m)= h(l,m,k)*temp2
...
!
         ...
         do 130 l = mf, jtrun
	  if (l.ne.1) then
            vorten(k,1,l,m)=-vorten(k,1,l,m)*omega/h(l,m,k)
            vorten(k,2,l,m)=-vorten(k,2,l,m)*omega/h(l,m,k)
            temp1= divten(k,2,l,m)*omega/h(l,m,k)
            temp2=-divten(k,1,l,m)*omega/h(l,m,k)
