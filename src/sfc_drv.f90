      ...
      SUBROUTINE SFC_DRV(imj,IM,KM,PS,U1,V1,T1,Q1,                   &
...
!       QS1(I) = EPS * QS1(I) / (PS1(I) + EPSM1 * QS1(I)) 
        call qsatq (1, t1(i), ps1(i)*0.01, qs1(i) )   !ps1 from pa to mb
...
!**      IF(SLOPETYP(I) .EQ. 0) SLOPETYP(I) = 2   !<--- QA(SLOPE) 
      ...
      IF(flag_iter(i).and. FLAG(I)) THEN 
!** 
!** GFS -> NOAH: PREPARE VARIABLES TO RUN NOAH LSM 
!** 
...
!   RADFLX     RADIATION FLUX (SOLDN or FDOWN)              
...
!0  CM          SURFACE EXCHANGE COEFFICIENT FOR MOMENTUM (M S-1)          -> CMX 
...
!*   H  CMC,T1,STC,SMC,SH2O,SNOWH,SNEQV,ALBEDO,CH,CM, 
        ...
        SOILW,SOILM,                                               &
        SMCWLT,SMCDRY,SMCREF,SMCMAX,NROOT,i,jj,io,jo) 
 
!** 
!**  NOAH -> GFS: PREPARE VARIABLES FOR RETURN TO PARENT MODE 
!** 
!   6. OUTPUT (O): 
...
!lu_q2m_iter [+17L]: restore land-related prognostic fields for guess run 
      do i=1, imj 
      IF(FLAG(I)) THEN 
