...
!
      ...
      real      work_io( ( ( (7+2*ncld)*2*                              &
                lev+8)*jtrun*jtmax+my*lev)*nsize)

      real      vornow(lev,2,jtrun,jtmax),divnow(lev,2,jtrun,jtmax)     &
      ...
      , trefs(lev,2,jtrun,jtmax) 

      real      plnow(jtrun,jtmax,2), plold(jtrun,jtmax,2)              &
...
!!      indx=indx+len1w
      call mpe_scatter_io(work_io(1+  indx *ns),uzm   ,len3,ns)
!
      return
      end
