...
!----------------------------------------------------------------------------------------------
      subroutine mpe2d_unify_tranuv_levp(ain,aout,lev,levp,jtmax,js,mlistnum,proc,comm,num)

! unify lev of grid space

      use mpi
      use index, only : mlist

      implicit none

      integer lev,levp,jtrun,jtmax,mlistnum,proc,comm,ii,j,jj,js,k,kk &
             ,ierr,num,n

      real ain(levp,2,num,jtmax,js)
      real aout(lev,2,num,jtmax,js)
!      real b1(levp,2,num,jtrun,jtmax)
      real b2(levp,2,num,jtmax,js,proc)

      aout = 0.
      b2   = 0.
      call MPI_ALLGATHER( ain, levp*2*num*jtmax*js,MPI_DOUBLE_PRECISION, &
                           b2, levp*2*num*jtmax*js,MPI_DOUBLE_PRECISION, &
                          comm,IERR )
      ...
      do jj=1,mlistnum
        do j=1,js
          do n=1,num
            kk=1
            do ii=1,proc
              aout(kk:kk+levp-1,1,n,jj,j)=b2(1:levp,1,n,jj,j,ii)
              aout(kk:kk+levp-1,2,n,jj,j)=b2(1:levp,2,n,jj,j,ii)
              kk=kk+levp
            enddo
          enddo
        enddo
...
!----------------------------------------------------------------------------------------------
      subroutine mpe2d_unify_spec_levp(ain,aout,lev,levp,jtrun,jtmax,mlistnum,proc,comm,num)
...
! unify lev of spectrum
      ...
      real ain(levp,2,num,jtrun,jtmax)
      real aout(lev,2,num,jtrun,jtmax)
!      real b1(levp,2,num,jtrun,jtmax)
      real b2(levp,2,num,jtrun,jtmax,proc)

      aout = 0.
      b2   = 0.
      call MPI_ALLGATHER( ain, levp*2*num*jtrun*jtmax,MPI_DOUBLE_PRECISION, &
                           b2, levp*2*num*jtrun*jtmax,MPI_DOUBLE_PRECISION, &
                          comm,IERR )
