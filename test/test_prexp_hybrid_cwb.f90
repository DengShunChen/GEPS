...
subroutine prexp_hybrid_cwb_unit
   ...
   !$acc enter data copyin(sigma, ptm, pk_gpu, pk2_gpu, plt_gpu, jlist1, nxjp) async(async_id)
   !$acc wait(async_id)
   do i = 1, steps
   ...
   !$acc wait(async_id)
   !$acc exit data copyout(pk_gpu, pk2_gpu, plt_gpu) delete(sigma, ptm, jlist1, nxjp) async(async_id)
   !$acc wait(async_id)

   if (all(abs(pk - pk_gpu) <= 1e-10) .AND. all(abs(pk2 - pk2_gpu) <= 1e-10) .AND. all(abs(plt - plt_gpu) <= 1e-10)) then
      print *, "test_prexp_hybrid_cwb passed."
   else
      print *, "test_prexp_hybrid_cwb failed."
      call exit(1)
   end if
