...
subroutine trngra3_unit
   ...
   !$acc enter data copyin(mtrundef, jlist1, jlist2, nlist, mlist, nxdef) async(async_id)
   !$acc enter data copyin(cim, poly, dpoly, s, dlpl_gpu, dtpl_gpu, cc, gwk1) async(async_id)
   do i = 1, steps
      call trngra3_gpu(jtrun, jtmax, nx, levp, my, my_max, cim, poly, dpoly, s, dlpl_gpu, dtpl_gpu, nsizey, cc, gwk1)
   end do
   !$acc exit data copyout(dlpl_gpu, dtpl_gpu) delete(cim, poly, dpoly, s, cc, gwk1) async(async_id)
   !$acc exit data delete(mtrundef, jlist1, jlist2, nlist, mlist, nxdef) async(async_id)
   !$acc wait(async_id)
